...
' these need to be broken out for separate form fields
			...
			if rsEvents("in_bba") = 1 then
				strBBA = " checked"
			else
				strBBA = ""
			end if
			
			if rsEvents("in_mba") = 1 then
				strMBA = " checked"
			else
				strMBA = ""
			end if
			
			if rsEvents("in_staff") = 1 then
				strStaff = " checked"
			else
				strStaff = ""
			end if
...
<form name="editForm" method="post" action="webCal4_staff-updated.asp?view=<%=Request.QueryString("view")%>">
...
<tr>
	...
	<td rowspan=2 width=256 valign="top"><font face="Tahoma, Arial, Helvetica" color="#<%=arColor(14)%>"><b>Description</b></font><br>
		<textarea cols="24" name="description" type="text" rows="10" wrap="virtual"><%=strDescription%></textarea>
	</td>
<tr>
	<td valign="top">
...
<!-- timing table -->
	...
	<tr>
		<td bgcolor="#<%=arColor(12)%>"><font face="Tahoma, Arial, Helvetica">
			...
			<br>
			<input name="start_date" id="date" type="text" size="13" value="<%=startDate%>"><font size=2><input type="button" value="&gt;" onClick="calpopup(2);">
			<br>
			Recurrence<br>
<%
...
<!-- end timing table -->
	...
	<tr>
		...
		<input type="checkbox" name="bba"<%=strBBA%>>undergrads
		<input type="checkbox" name="mba"<%=strMBA%>>grads
		</font></td>
