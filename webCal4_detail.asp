...
dim strDate, strStart, strEnd, strView, strType
...
<tr>
	...
	<tr>
		<td bgcolor="#<%=arColor(4)%>">
			...
			<a href="webCal4_<%=strType%>-<%=strView%>.asp?date=<%=eventDate%>"
...
if strDescription <> "" then
	response.write "<tr><td><font size=2>" _
		& Replace(strDescription, VbCrLf, "<br>") _
		& "</font></td>"
end if
%>
...
<% if CStr(intUser) = CStr(Session("StudentID")) then %>
...
<tr>
	...
	<tr>
		...
		<form action="webCal4_<%=strCal%>-edit.asp?action=form" method="post">
...
<%		end if %>
	...
	<input type="hidden" name="view" value="<%=strView%>">
