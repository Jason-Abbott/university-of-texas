...
dim dayShow, strQuery, intTime, strView, intID, strCal
...
' determine the level of event detail to reveal
	...
	if intID = Session("StudentID") then
		arEvents(d,intTime,0) = "<a href=""webCal4_detail.asp?event_id=" _
			...
			& "&type=" & strCal & "&view=week"" " & showStatus(strDescription) & ">" _
...
<!--#include file="webCal4_counsel-buttons.inc"-->
<tr>
...
%>
	<tr>
		...
		<a href="webCal4_<%=strCal%>-month.asp?date=<%=strFirst%>&staff_id=<%=intID%>"
...
<% if arSpan(0) < intDays + 1 then %>
		...
		<font face="Tahoma, Arial, Helvetica" size=2 color="#<%=arColor(6)%>">
		<a href="webCal4_<%=strCal%>-month.asp?date=<%=strLast%>&staff_id=<%=intID%>"
